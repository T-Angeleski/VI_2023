...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        newState = moveForward(list(snake_body), list(snake_head),
                               list(green_apples))
        if self.check_valid(newState):
            successors["ProdolzhiPravo"] = \
                (tuple(newState[0]), tuple(newState[1]), tuple(newState[2]))

        newState = turnRight(list(snake_body), list(snake_head),
                             list(green_apples))
        if self.check_valid(newState):
            successors["SvrtiDesno"] = \
                (tuple(newState[0]), tuple(newState[1]), tuple(newState[2]))

        newState = turnLeft(list(snake_body), list(snake_head),
                            list(green_apples))
        if self.check_valid(newState):
    ...
    def check_valid(self, state):
        ...
        if (head[0], head[1]) in body: return False
        if (head[0], head[1]) in self.red_apples: return False
...
if __name__ == '__main__':
    ...
    snake = Snake(redApples, (snakeBody, snakeHead, tuple(greenApples)))
    solution = breadth_first_graph_search(snake).solution()
    print(solution)