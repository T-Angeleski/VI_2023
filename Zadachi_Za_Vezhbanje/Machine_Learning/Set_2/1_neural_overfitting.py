...
def divideSets(train, validation):
	tx = [row[:-1] for row in train]
	ty = [row[-1] for row in train]
	vx = [row[:-1] for row in validation]
	vy = [row[-1] for row in validation]
	
	return tx, ty, vx, vy


def calculateAccuracy(pred, real):
	acc = 0
	for predicted, true in zip(pred, real):
		if predicted == true: acc += 1
	return acc / len(real)
...
if __name__ == '__main__':
	...
	train, validation = divideData(dataset)
	trainX, trainY, valX, valY = divideSets(train, validation)
	
	classifier1 = MLPClassifier(epoch, activation="relu", max_iter=20,
	...
	classifier1.fit(scaler.transform(trainX), trainY)
	
	trainPred = classifier1.predict(scaler.transform(trainX))
	valPred = classifier1.predict(scaler.transform(valX))
	
	trainAcc = calculateAccuracy(trainPred, trainY)
	valAcc = calculateAccuracy(valPred, valY)
	
	if valAcc * 1.15 <= trainAcc:
		print("Se sluchuva overfitting")
		for row in trainX:
			row.pop(index)
		
		classifier2 = MLPClassifier(epoch, activation="relu", max_iter=20,
		                            learning_rate_init=learningRate,
		                            random_state=0)
		
		scaler2 = MinMaxScaler(feature_range=(-1, 1))
		scaler2.fit(trainX)
		classifier2.fit(scaler2.transform(trainX), trainY)
		entry.pop(index)
		predictedEntry = classifier2.predict(scaler2.transform([entry]))[0]
		print(predictedEntry)
	
	else:
		predictedEntry = classifier1.predict(scaler.transform([entry]))[0]
