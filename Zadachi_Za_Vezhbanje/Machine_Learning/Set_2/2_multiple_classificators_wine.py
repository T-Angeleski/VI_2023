...
dataset = [
	...
	[13.24, 2.59, 2.87, 21, 118, 2.8, 2.69, .39, 1.82, 4.32, 1.04, 2.93, 735,
	 0],
	[14.2, 1.76, 2.45, 15.2, 112, 3.27, 3.39, .34, 1.97, 6.75, 1.05, 2.85,
	 1450, 0],
	[14.39, 1.87, 2.45, 14.6, 96, 2.5, 2.52, .3, 1.98, 5.25, 1.02, 3.58, 1290,
	 0],
	[14.06, 2.15, 2.61, 17.6, 121, 2.6, 2.51, .31, 1.25, 5.05, 1.06, 3.58,
	 1295, 0],
...
def getTrainAndTest(a, b, oneClass, twoClass, zeroClass):
	trainNB = zeroClass[:int(a * len(zeroClass))] + \
	          oneClass[:int(a * len(oneClass))] + \
	          twoClass[:int(a * len(twoClass))]
	trainDT = zeroClass[int(a * len(zeroClass)):int(b * len(zeroClass))] + \
	          oneClass[int(a * len(oneClass)):int(b * len(oneClass))] + \
	          twoClass[int(a * len(twoClass)):int(b * len(twoClass))]
	trainRF = zeroClass[:int(b * len(zeroClass))] + \
	          oneClass[:int(b * len(oneClass))] + \
	          twoClass[:int(b * len(twoClass))]
	test = zeroClass[int(b * len(zeroClass)):] + \
	       oneClass[int(b * len(oneClass)):] + \
	       twoClass[int(b * len(twoClass)):]
	
	return test, trainDT, trainNB, trainRF


def divideData(data, a, b):
	zeroClass, oneClass, twoClass = [], [], []
	for row in data:
		if row[-1] == 0: zeroClass.append(row)
		elif row[-1] == 1: oneClass.append(row)
		else: twoClass.append(row)
	
	test, trainDT, trainNB, trainRF = getTrainAndTest(a, b, oneClass, twoClass,
	                                                  zeroClass)
	
	return trainNB, trainDT, trainRF, test
...
def divideSets(trainNB, trainDT, trainRF, test):
	NBX, NBY = splitSet(trainNB)
	DTX, DTY = splitSet(trainDT)
	RFX, RFY = splitSet(trainRF)
	TX, TY = splitSet(test)
	
	return NBX, NBY, DTX, DTY, RFX, RFY, TX, TY


def calculate(predictionsNB, predictionsDT, predictionsRF, testY):
	correct = 0
	for nb, dt, rf, real in zip(predictionsNB, predictionsDT, predictionsRF,
	                            testY):
		count = 0
		if nb == real: count += 1
		if dt == real: count += 1
...
if __name__ == '__main__':
	...
	trainNB, trainDT, trainRF, test = divideData(dataset, a, b)
	NBX, NBY, DTX, DTY, RFX, RFY, TX, TY = divideSets(trainNB, trainDT,
	                                                 trainRF, test)
	classifierNB = GaussianNB()
	classifierDT = DecisionTreeClassifier(random_state=0)
	classifierRF = RandomForestClassifier(n_estimators=3, random_state=0)
	
	classifierNB.fit(NBX, NBY)
	classifierDT.fit(DTX, DTY)
	classifierRF.fit(RFX, RFY)
	
	predictionsNB = classifierNB.predict(TX)
	predictionsDT = classifierDT.predict(TX)
	predictionsRF = classifierRF.predict(TX)
	
	accuracy = calculate(predictionsNB, predictionsDT, predictionsRF, TY)
