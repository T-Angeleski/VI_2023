...
if __name__ == '__main__':
	...
	updatedData = []
	for row in dataset:
		newRow = [row[i] for i in range(len(row)) if i != removeID]
		updatedData.append(newRow)
	
	divide = int(0.85 * len(updatedData))
	train = updatedData[:divide]
	test = updatedData[divide:]
	...
	classifier = RandomForestClassifier(numTrees, criterion=criterion,
	                                    random_state=0)
	
	classifier.fit(trainX, trainY)
	
	acc = 0
	for pred, true in zip(classifier.predict(testX), testY):
		if pred == true: acc += 1
	acc /= len(testY)
	
	entry.pop(removeID)
	predicted = classifier.predict([entry])[0]
