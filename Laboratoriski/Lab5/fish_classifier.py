...
if __name__ == '__main__':
	columnToRemove = int(input())
	numTrees = int(input())
	criteriumType = input()
	
	updatedDataset = []
	for row in dataset:  # Pass through each entry in data
		# Check if entry[i] to be removed
		tmp = [row[i] for i in range(len(row)) if i != columnToRemove]
		updatedDataset.append(tmp)
	
	dividingPoint = int(0.85 * len(updatedDataset))  # 85/15
	trainSet = updatedDataset[:dividingPoint]
	testSet = updatedDataset[dividingPoint:]
	...
	for X, realValue in zip(testX, testY):
		predicted = classifier.predict([X])[0]
		if predicted == realValue: correctlyPredicted += 1
	
	accuracy = correctlyPredicted / len(testSet)
	print(f"Accuracy: {accuracy}")
	
	inputEntry = [float(el) for el in input().split(" ")]
	inputEntry.pop(columnToRemove)
	print(classifier.predict([inputEntry])[0])
	print(classifier.predict_proba([inputEntry])[0])