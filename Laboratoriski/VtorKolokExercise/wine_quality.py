...
dataset = [
	[9.2, 0.755, 0.18, 2.2, 0.148, 10.0, 103.0, 0.9969, 2.87, 1.36, 10.2,
	 8.476],
	...
	[10.1, 0.935, 0.22, 3.4, 0.105, 11.0, 86.0, 1.001, 3.43, 0.64, 11.3,
	 0.133],
	...
	[9.5, 0.885, 0.27, 2.3, 0.084, 31.0, 145.0, 0.9978, 3.24, 0.53, 9.4,
	 0.794],
	...
	[10.0, 0.73, 0.43, 2.3, 0.059, 15.0, 31.0, 0.9966, 3.15, 0.57, 11.0,
	 1.467],
	...
	[10.6, 1.02, 0.43, 2.9, 0.076, 26.0, 88.0, 0.9984, 3.08, 0.57, 10.1,
	 7.425],
	[7.0, 0.65, 0.02, 2.1, 0.066, 8.0, 25.0, 0.9972, 3.47, 0.67, 9.5, 9.568],
	[7.0, 0.64, 0.02, 2.1, 0.067, 9.0, 23.0, 0.997, 3.47, 0.67, 9.4, 8.263],
	[10.4, 0.44, 0.42, 1.5, 0.145, 34.0, 48.0, 0.99832, 3.38, 0.86, 9.9,
	 3.491],
	[11.6, 0.47, 0.44, 1.6, 0.147, 36.0, 51.0, 0.99836, 3.38, 0.86, 9.9,
	 0.532],
	[8.8, 0.685, 0.26, 1.6, 0.088, 16.0, 23.0, 0.99694, 3.32, 0.47, 9.4,
	 0.944],
	[7.6, 0.665, 0.1, 1.5, 0.066, 27.0, 55.0, 0.99655, 3.39, 0.51, 9.3, 0.785],
	[6.7, 0.28, 0.28, 2.4, 0.012, 36.0, 100.0, 0.99064, 3.26, 0.39, 11.7,
	 5.764],
	[6.7, 0.28, 0.28, 2.4, 0.012, 36.0, 100.0, 0.99064, 3.26, 0.39, 11.7,
	 8.84],
	[10.1, 0.31, 0.35, 1.6, 0.075, 9.0, 28.0, 0.99672, 3.24, 0.83, 11.2,
	 6.921],
	[6.0, 0.5, 0.04, 2.2, 0.092, 13.0, 26.0, 0.99647, 3.46, 0.47, 10.0, 3.465],
	[11.1, 0.42, 0.47, 2.65, 0.085, 9.0, 34.0, 0.99736, 3.24, 0.77, 12.1,
	 9.32],
	[6.6, 0.66, 0.0, 3.0, 0.115, 21.0, 31.0, 0.99629, 3.45, 0.63, 10.3, 3.078],
	[10.6, 0.5, 0.45, 2.6, 0.119, 34.0, 68.0, 0.99708, 3.23, 0.72, 10.9,
	 5.076],
	...
	[7.6, 0.735, 0.02, 2.5, 0.071, 10.0, 14.0, 0.99538, 3.51, 0.71, 11.7,
	 8.02],
	...
	[11.7, 0.28, 0.47, 1.7, 0.054, 17.0, 32.0, 0.99686, 3.15, 0.67, 10.6,
	 5.097],
	...
	[7.2, 0.635, 0.07, 2.6, 0.077, 16.0, 86.0, 0.99748, 3.51, 0.54, 9.7,
	 3.773],
	[6.8, 0.49, 0.22, 2.3, 0.071, 13.0, 24.0, 0.99438, 3.41, 0.83, 11.3,
	 9.929],
	...
	[7.7, 0.715, 0.01, 2.1, 0.064, 31.0, 43.0, 0.99371, 3.41, 0.57, 11.8,
	 6.849],
	[6.9, 0.56, 0.03, 1.5, 0.086, 36.0, 46.0, 0.99522, 3.53, 0.57, 10.6,
	 0.484],
	[7.3, 0.35, 0.24, 2.0, 0.067, 28.0, 48.0, 0.99576, 3.43, 0.54, 10.0,
	 3.679],
	[9.1, 0.21, 0.37, 1.6, 0.067, 6.0, 10.0, 0.99552, 3.23, 0.58, 11.1, 8.272],
	[10.4, 0.38, 0.46, 2.1, 0.104, 6.0, 10.0, 0.99664, 3.12, 0.65, 11.8,
	 8.446],
	...
	[7.6, 0.715, 0.0, 2.1, 0.068, 30.0, 35.0, 0.99533, 3.48, 0.65, 11.4,
	 6.845]]
...
def divideData(data, divide):
	# First X for testing
	dividePoint = divide * 0.01 * len(data)
	testSet = data[:int(dividePoint)]
	trainSet = data[int(dividePoint):]
	
	return trainSet, testSet


def divideSets(trainSet, testSet):
...
def getAccuracy(predictions, test):
	count = 0
	for pred, real in zip(predictions, test):
		if pred == real: count += 1
...
if __name__ == '__main__':
	...
	newDataset = transformData(dataset)
	trainSet, testSet = divideData(newDataset, x)
	trainX, trainY, testX, testY = divideSets(trainSet, testSet)
	
	classifierDT = DecisionTreeClassifier(criterion="gini", random_state=0)
	classifierDT.fit(trainX, trainY)
	
	# Remove least important feature
	allFeatures = list(classifierDT.feature_importances_)
	leastImportant = allFeatures.index(min(allFeatures))
	featureRemovedDataset = []
	for row in newDataset:
		newRow = [row[i] for i in range(len(row)) if i != leastImportant]
		featureRemovedDataset.append(newRow)
	
	classifierNN1 = MLPClassifier(15, activation="relu",
	                             learning_rate_init=0.001, max_iter=200,
	                             random_state=0)
	classifierNN2 = MLPClassifier(15, activation="relu",
	                              learning_rate_init=0.001, max_iter=200,
	                              random_state=0)
	
	train2, test2 = divideData(featureRemovedDataset, x)
	trainX2, trainY2, testX2, testY2 = divideSets(train2, test2)
	
	scalerStandard = StandardScaler()
	scalerMinMax = MinMaxScaler()
	
	scalerStandard.fit(trainX2)
	scalerMinMax.fit(trainX2)
	
	classifierNN1.fit(scalerStandard.transform(trainX2), trainY2)
	classifierNN2.fit(scalerMinMax.transform(trainX2), trainY2)
	
	predictionsStandard = classifierNN1.predict(scalerStandard.transform(testX2))
	predictionsMinMax = classifierNN2.predict(scalerMinMax.transform(testX2))
	
	accuracyStandard = getAccuracy(predictionsStandard, testY2)
	accuracyMinMax = getAccuracy(predictionsMinMax, testY2)
	
	print(f"Tocnost so StandardScaler: {accuracyStandard}")
	print(f"Tocnost so MinMaxScaler: {accuracyMinMax}")